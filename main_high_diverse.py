...
from Server.server_1_high_diverse import process_encryption
import pytroy
import random
import numpy as np
import pickle
...
splits_dict = load_mappings_from_file_grid(os.path.join(data_dir, f'test_splits_dict{d_value}.pkl'))
grid_mappings = load_mappings_from_file(os.path.join(data_dir, f'grid_mappings{d_value}.pkl'))
# if k<=5:
independent_mappings = load_mappings_from_file(os.path.join(data_dir, f'independent_mappings{d_value}.pkl'))
# elif k>5:
...
file_path='Data/vr_id_to_coordinates_AES{d_value}.pkl'
with open(file_path, "rb") as file:
