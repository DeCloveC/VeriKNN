...
def calcute_grid(ckks,encrpted_gird,length,mac1,encrypted_timestamp_tag,num_to_change = 2):
    ...
    if result:
        ...
        # print("mul_d",mul_d)
        # print("mul_d",mul_d)
        values_per_cell_check = 2 * gamma
        
        mul_d=mul_d[:length]
        num_cells_in_batch = length // values_per_cell_check
        temp = [0] *num_cells_in_batch
        for i in range(num_cells_in_batch):
            # Calculate the starting index for the current cell's comparison results in mul_d
            start_index = i * values_per_cell_check

            # Check if the point is within the current cell
            # A point is inside if (point_coord - min_coord) >= 0 AND (max_coord - point_coord) >= 0 for ALL dimensions.
            is_point_in_current_cell = True
            # Using a small tolerance (e.g., 1e-6) for floating point comparisons from FHE decryption
            tolerance = 1e-6 

            for dim_idx in range(gamma):
                # Get the comparison results for the current dimension
                # Assuming mul_d is structured as [ (p0-min0), (max0-p0), (p1-min1), (max1-p1), ... ]
                val_p_minus_min = mul_d[start_index + (2 * dim_idx)]
                val_max_minus_p = mul_d[start_index + (2 * dim_idx + 1)]

                # Check the conditions for this dimension: (p_j - min_j >= 0) AND (max_j - p_j >= 0)
                # Using tolerance for floating point numbers
                if not (val_p_minus_min >= -tolerance and val_max_minus_p >= -tolerance):
                    is_point_in_current_cell = False
                    break # If any dimension fails, the point is not in the cell

            if is_point_in_current_cell:
                temp[i] = 1 # Mark this cell as containing the point
        # print("temp",temp)
        temp=np.array(temp).reshape(-1, 1).flatten().tolist()
        zero_indices = [i for i, value in enumerate(temp) if value == 0]
        # print(zero_indices)
        if len(zero_indices) == num_cells_in_batch:
            # print('none')
            return [],None,None
        else:
...
def calcute_grid_ou(ckks, encrpted_ou_first, VR_id_list,mac2,encrypted_timestamp_tag):
    ...
    if result:
        ...
        for row_index, encrypted_ou in enumerate(decrypted_ou_first):
            ...
            # 找到当前加密数据的最小值及其下标
            local_min_value = min(flattened_mul_d)
            local_min_index = flattened_mul_d.index(local_min_value)
