import random
from Server.server_2 import calcute_grid,calcute_grid_ou_final,calcute_grid_ou
...
def process_neighbors_and_compute(ckks, min_index, mappings, enc_x, enc_y, plain_data_r, enc_point, k):
    ...
    if neighbors:
        ...
        # 获取加密的网格数据
        encrpt_gird_vr_x_point = neighbors['First Coordinates List']
        encrpt_gird_vr_y_point = neighbors['Second Coordinates List']

        # 进行加密计算：mod_switch、加法、平方、乘法等
        ckks.evaluator.mod_switch_to_inplace(encrpt_gird_vr_x_point, enc_point.parms_id())
        
        # 密文相加
        enc_add_x = ckks.evaluator.add(encrpt_gird_vr_x_point, enc_x)
        enc_add_x = ckks.evaluator.square(enc_add_x)

        enc_add_y = ckks.evaluator.add(encrpt_gird_vr_y_point, enc_y)
        enc_add_y = ckks.evaluator.square(enc_add_y)

        # 将加法结果相加
        enc_add_final = ckks.evaluator.add(enc_add_x, enc_add_y)

        # 将加法结果与明文数据相乘
        enc_add_final = ckks.evaluator.multiply_plain(enc_add_final, plain_data_r)
...
def process_encryption(ckks, enc_point, enc_x, enc_y, k=1,
    ...
    # 初始化 plain_grid
        ...
        min_value = -30
        max_value = -1
        possible_values = [x for x in range(min_value, max_value + 1) if x % -4 == 0]
        ...
        plain_grid,mac,encrypted_timestamp_tag = calcute_grid(ckks, temp_rot, length * 4 - t, generated_mac,encrypted_timestamp_tag,num_to_change=2 )
        ...
        if mac:
            ...
            plain_grid=plain_grid[-t//4:(length * 4 - t)//4]
            # raise ValueError
            # print('there2')
            if not result:
                print(message)
            ...
            for idx in random_indices:
                ...
                encrpt_gird_vr_x_point = grid_encode[idx]['First Coordinates List']  # 这里假设 'Grid_X' 是对应的数据
                encrpt_gird_vr_y_point = grid_encode[idx]['Second Coordinates List']  # 假设 'Grid_Y' 是对应的数据
                
                # 加密计算过程
                ckks.evaluator.mod_switch_to_inplace(encrpt_gird_vr_x_point, enc_point.parms_id())
                ckks.evaluator.mod_switch_to_inplace(encrpt_gird_vr_y_point, enc_point.parms_id())
